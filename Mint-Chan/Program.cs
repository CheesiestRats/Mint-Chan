...
namespace Mint_Chan
{
    internal class Program
    {
        ...
        internal static string botLastReply = string.Empty;
        ...
        internal static string characterPrompt = $@""; // you can use the below format to make a character prompt
        ...
        internal static string bannedWordsExact = @"\b(naked|boobies|meth|adult video)\b";
        ...
        internal static string takeAPicRegexStr = @"\b(take|paint|generate|make|draw|create|show|give|snap|capture|send|display|share|shoot|see|provide|another)\b.*(\S\s{0,10})?(image|picture|painting|pic|photo|portrait|selfie)\b";
        string promptEndDetectionRegexStr = @"(?:\r\n?|\n)(?:(?![.\-*]).){2}|(\n\[|\[end|<end|]:|>:|\[human|\[chat|\[minty|\[cc|<chat|<cc|\[@chat|\[@cc|bot\]:|<@chat|<@cc|\[.*]: |\[.*] : |\[[^\]]+\]\s*:)";
        ...
        readonly string linkDetectionRegexStr = @"[a-zA-Z0-9]((?i) dot |(?i) dotcom|(?i)dotcom|(?i)dotcom |\.|\. | \.| \. |\,)[a-zA-Z]*((?i) slash |(?i) slash|(?i)slash |(?i)slash|\/|\/ | \/| \/ ).+[a-zA-Z0-9]";

        private readonly Regex _takeAPicRegex = new Regex(takeAPicRegexStr, RegexOptions.IgnoreCase);
        ...
        private async Task TakeAPic(SocketUserMessage Msg, string llmPrompt, string userPrompt)
        {
            ...
            DateTime currentTime = Functions.GetCurrentTime();
            string timeOfDayInNaturalLanguage = Functions.GetTimeOfDay(currentTime);
